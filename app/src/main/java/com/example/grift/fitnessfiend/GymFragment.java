...
public class GymFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        timeEditText.setOnClickListener(view13 -> {
        ...
        if (FirebaseAuth.getInstance().getCurrentUser() != null) {
            ...
            reference.child("gym_reminder_screen").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
        ...
        setReminderButton.setOnClickListener(view12 -> {
            if (FirebaseAuth.getInstance().getCurrentUser() != null) {
                DatabaseReference reference = FirebaseDatabase.getInstance().getReference().child("users").child(
                    FirebaseAuth.getInstance().getCurrentUser().getUid());
                reference.child("gym_reminder_screen").child("date").setValue(dateEditText.getText().toString());
                reference.child("gym_reminder_screen").child("time").setValue(timeEditText.getText().toString());
            }
            Toast.makeText(getContext(), "Reminder set", Toast.LENGTH_LONG).show();
            Intent intent = new Intent(getActivity(), GymReminderBroadcast.class);
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            CharSequence name = "GymReminderChannel";
            String description = "Channel for Gym Reminder";
